const SECRET = process.env.SECRET || "8b6e4a97-bfe5-43b4-bdf0-cbad4fa7261c";// just for offline operations
...
module.exports.getLastLookbackHigh =(highs, lookback) =>{
...
module.exports.getLastLookbackLow  =(lows, lookback) =>{
    const result = [];
...
/**
 * Risk-based position size, in base-currency units.
 *
 * Sizes a position so that moving from `entryPrice` to `stopPrice` costs exactly
 * `riskPct` of `equity`. This is what lets a strategy with a positive per-trade edge
 * survive geometrically: a fixed notional risks a wildly different fraction of equity
 * on a tight stop than on a wide one, and the wide-stop trades are what produce the
 * deep drawdowns.
 *
 * The result is capped at `maxNotionalMult` x equity so the caller can never be sized
 * into leverage it did not ask for (a very tight stop otherwise implies a huge position).
 *
 * @param equity {number} account equity in quote currency
 * @param entryPrice {number} the intended entry fill price
 * @param stopPrice {number} the protective stop price
 * @param riskPct {number} fraction of equity to risk, e.g. 0.01 for 1%
 * @param maxNotionalMult {number} cap on notional as a multiple of equity (default 1 = no leverage)
 * @returns {number|null} size in base units, or null when the inputs cannot produce one
 *                        (callers should fall back to their existing amount)
 */
module.exports.riskPositionSize = (equity, entryPrice, stopPrice, riskPct, maxNotionalMult = 1) =>{
    if(!Number.isFinite(equity) || !Number.isFinite(entryPrice) || !Number.isFinite(stopPrice)) return null;
    if(!Number.isFinite(riskPct) || equity <= 0 || entryPrice <= 0 || riskPct <= 0) return null;
    let stopDistance = Math.abs(entryPrice - stopPrice);
    if(!(stopDistance > 0)) return null;
    let size = (equity * riskPct) / stopDistance;
    let maxSize = (equity * maxNotionalMult) / entryPrice;
    return Math.min(size, maxSize);
}

module.exports.requiredCandlesForPattern =() =>{
    return{
        AbandonedBaby: 3,
        ...
        BullishHammer: 1,
        BearishHammer: 1,
        BullishInvertedHammer: 1,
        BearishInvertedHammer: 1,
        HammerPattern: 5,
