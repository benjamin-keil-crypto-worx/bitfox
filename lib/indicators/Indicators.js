...
class DonchianIndicator{
    ...
    static className = "DonchianIndicator"
    /**
     * Rolling Donchian channel.
     *
     * The channel at each output row is built from the `period` bars that PRECEDE that bar
     * and deliberately excludes the bar itself. A breakout has to be measured against a
     * channel the current bar did not help create — if the current high were included,
     * `close > upper` could never be true and the indicator would emit no signals at all.
     *
     * @param o {Array<number>}  The Opening Candles
     * @param h {Array<number>}  The Higher High Candles
     * @param l {Array<number>}  The Lower Low Candles
     * @param c {Array<number>}  The Closing Candles
     * @param v {Array<number>}  The Volume Data
     * @param args.period {number} (the channel lookback, default 20)
     * @param candles {Array<Array<number>>} o,h,l,c,v array Buffer
     * @returns {Array<{upper:number, lower:number, middle:number}>} one row per bar after the warm-up
     * */
    static getData(o,h,l,c,v, args, candles){
        let period = (args && args.period) || 20;
        let result = [];
        if(!h || !l || h.length <= period) return result;
        for(let i = period; i < h.length; i++){
            let upper = -Infinity;
            let lower = Infinity;
            for(let j = i - period; j < i; j++){
                if(h[j] > upper) upper = h[j];
                if(l[j] < lower) lower = l[j];
            }
            result.push({upper: upper, lower: lower, middle: (upper + lower) / 2});
        }
        return result;
    }
}
...
module.exports = {
    ...
    DonchianIndicator:DonchianIndicator,
