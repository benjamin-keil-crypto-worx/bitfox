...
class DynamicGridSignals extends CustomIndicator {
    ...
    constructor(o,h,l,c,v, args, candles){
        ...
        this.lookback = args.lookback || 200;
        this.grids = args.grid || 5;
    }

    getLastLookbackHighs(highs){
        let allLookBackHighs = highs.splice(0,this.lookback);
        // Sort the Array ascending
        allLookBackHighs.sort(function  (a,b){  return  a.high - b.high; });
        allLookBackHighs = allLookBackHighs.splice(allLookBackHighs.length-this.grids)
        return allLookBackHighs;
    }
    getLastLookbackLows(lows){
        let allLookBackLows = lows.splice(0,this.lookback);
        // Sort the Array descending
        allLookBackLows.sort(function  (a,b){  return  b.low - a.low; });
        return allLookBackLows;
    }
    ...
    calculate(){
        ...
        while(highs.length>0){
            highBuffer.push(this.getLastLookbackHighs(highs)) 
        }
        while(lows.length>0){
            lowBuffer.push(this.getLastLookbackLows(lows))
        }

        return {grids:{high:highBuffer, lows:lowBuffer}}
    }
}
...
class MultiDivergenceDetector extends CustomIndicator {
    ...
    detectMacdDivergence(highBuffer, lowBuffer){
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (currentClose < previousLow && currentMacdLine > previousMacdOnLow) {
            lowBufferStartIndex++;
            highBufferStartIndex++;
            this.divergenceSignals.push({
    ...
    detectStochDivergence(highBuffer, lowBuffer){
        ...
        for (let i = 0; i < length; i++) {
          ...
          // Bearish Divergence: Lower close, Higher RSI
          if (currentClose < previousLow && currentStochLine > previousStochOnLow) {
            lowBufferStartIndex++;
            highBufferStartIndex++;
            this.divergenceSignals.push({
              type: 'MACD Bullish Divergence',
    ...
    detectWilliamsRDivergence(highBuffer, lowBuffer){
        ...
        for (let i = 0; i < length; i++) {
          ...
          // Bearish Divergence: Lower close, Higher RSI
          if (currentClose < previousLow && currentWR > previousWRIOnLow) {
            lowBufferStartIndex++;
            highBufferStartIndex++;
            this.divergenceSignals.push({
    ...
    detectRSIDivergence(highBuffer, lowBuffer) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          // Bearish Divergence: Lower close, Higher RSI
          if (currentClose < previousLow && currentRSI > previousRSIOnLow) {
            lowBufferStartIndex++;
            highBufferStartIndex++;
            this.divergenceSignals.push({
