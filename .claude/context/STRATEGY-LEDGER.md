...
| 4h | ✅ 07-31 | ✅ 07-31 | — | — | — | |
| 1h | ✅ 07-31 | ✅ 07-31 | — | — | — | |
| **15m** | **✅ 08-01** | **✅ 08-01** | **✅ 08-01** | **◐ 08-02** | **◐ 08-02** | + TP/SL grid on BTC/ADA/SOL; ◐ = Bollinger only (also DOGE/LINK/LTC) |
| **5m** | **✅ 08-01** | **✅ 08-01** | **✅ 08-01** | **◐ 08-02** | **◐ 08-02** | as above |
...
DonchianTrend at 15m is catastrophic, which is expected — it is a 1d strategy.

### Bollinger settle (2026-08-02) — SETTLED, no-go

The BTC 15m result above briefly made Bollinger the top lead. An 8-symbol sweep killed it. Honest engine, ~208 days at 15m / ~69 days at 5m, halves shown for a crude in/out split.

| Symbol | 15m n | 15m PF | 5m n | 5m PF |
|---|---|---|---|---|
| **BTC** | 72 | **1.26** | 28 | 0.78 |
| ETH | 108 | 0.96 | 50 | 0.62 |
| ADA | 102 | 0.96 | 40 | 0.51 |
| SOL | 102 | 0.86 | 41 | **1.50** |
| XRP | 92 | 0.83 | 29 | **1.31** |
| DOGE | 109 | 0.88 | 36 | 0.93 |
| LINK | 76 | 0.80 | 33 | 0.85 |
| LTC | 69 | 0.96 | 34 | 0.82 |
| **pooled** | **730** | **median 0.96 · 1/8 above 1.0** | **291** | **median 0.85 · 2/8 above 1.0** |

**Verdict: single-cell wonder, dies on ship-bar criterion 4.** BTC is the only 15m symbol above 1.0, and its own 5m cell is 0.78. The two positive 5m cells (SOL 1.50, XRP 1.31) are on n=41 and n=29 — below the 50-trade bar, and they are *not* the symbols that worked at 15m. Positives that move between symbols and timeframes are a noise signature, not a strategy.

**Trap worth remembering:** the second-half (recent-window) column looked encouraging — median PF 1.17 at 15m with 7/8 symbols above 1.0. That is **not** an OOS win. No parameters were fitted, so both halves are equally in-sample, and *every* symbol improved in the recent window — that is a regime effect favouring mean reversion, not evidence of edge. Half-samples were n=30–48, under the bar anyway. A uniform lift across all symbols indicates the market changed, not that the strategy works.
...
| Bollinger | **no-go — settled 2026-08-02** | BTC 15m PF 1.26 (n=72); pooled **median PF 0.96, 1/8 symbols positive, n=730** | Briefly the top lead when only BTC/ADA 15m were measured. The 8-symbol sweep killed it: **BTC 15m is a single-cell wonder**, failing ship-bar criterion 4. See the settle table below |
| Regime | **no-go** | BTC 4h PF 1.07 (vs B&H +272%) | Walk-forward OOS decisively negative (−70% 1h, −201% 4h); train winners unstable |
| Crocodile (4-EMA ribbon) | **no-go — never built** | Pooled 1d PF **1.07**, n=177, t=0.47 | GHBF-44, killed at probe stage 2026-08-01 before writing code. 20/50/100/200 EMA, edge-triggered. Monotonic in timeframe: 15m PF 0.64–0.81 (n≈330, WR 27%), 1h 0.44–1.00, 4h 0.70–1.12, 1d looked great at PF 2.30/3.65 but on **n=14–18**. Pooled across 10 symbols the 1d PF collapses to 1.07, **4 of 10 symbols positive** — a coin flip. "Confidence factor" (min ribbon width) made it monotonically *worse*: PF 1.07→0.99→0.97→0.95 as the filter tightened. Probe only, not an engine run — but negative enough that an engine run would not flip it |
| SuperTrend | no-go | BTC 4h PF 0.97 | PF 0.75–0.97 everywhere. **Note for anyone who remembers it "working":** the old PF 6.59 / +455% figures were 100% fill-model artifact (pre-GHBF-26 every exit filled at the bar's best price, so stopped-out longs exited at the candle high, often at a profit) |
...
- **"Confidence factors" / entry-strength filters as a fix for a weak signal** (probed 2026-08-01 on Crocodile). Requiring a minimum ribbon width (trend strength) at entry made results monotonically **worse**: PF 1.07 (n=177) → 1.06 (n=176) → 0.99 (n=168) → 0.97 (n=125) → 0.95 (n=66) as the threshold rose from 0% to 15%. The strongest-trend subset underperformed the full set. **Generalize the lesson, not the filter:** if a signal has no edge, concentrating it does not create one — and the more common failure mode is the opposite (a tight filter that inflates PF purely by shrinking n, cf. the pullback decay curve above). Either way, a filter that improves in-sample PF is evidence of nothing without walk-forward on a held sample size.
